...
1) changed inline styling from absolute to static. 
2) Inputcheckbox - placed input checkbox inside label tag.

3) App.tsx we need to call loadAlltransaction if All employees is selected. the new value for all employee is an empty string.        if (newValue.id === ''){
